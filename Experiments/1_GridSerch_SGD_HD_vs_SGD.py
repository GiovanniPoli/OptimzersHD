...
for i in range(6):
  for j in range(7):
    ...
    print_ = True
    if isnan(beta):
        opt = keras.optimizers.SGD(lr=alpha, momentum=0.0, nesterov=False)
    else: 
        opt = MyOpts.SGD_HD(alpha_0= alpha, beta= beta)
...
save_result(Grid_np, 'Output_GridSerch_SGD')
# Grid_np = load_result('Output_GridSerch_SGD')

fig, ax = plt.subplots(figsize=(18, 8))
    
xlab = ["$10^{-6}$", "$10^{-5}$", "$10^{-4}$",
        "$10^{-3}$","$10^{-2}$","$10^{-1}$","Ref. SGD"]
ylab = ["$10^{-1}$", "$10^{-2}$", "$10^{-3}$",
        "$10^{-4}$","$10^{-5}$","$10^{-6}$"]
...
plt.plot([2.5,2.5,4.5,4.5,5.45],[-0.5,0.5,0.5,1.5,1.5], linewidth=2, c='r')
plt.fill([2.5,2.5,4.5,4.5,5.5,5.5],[-0.5,0.5,0.5,1.5,1.5,-0.5],
         c='r', alpha=0.2)
plt.fill([5.5,5.5,6.5,6.5],[2.5,5.5,5.5,2.5],
         c='grey', alpha=1)
text = ax.text(6, 4, " More then \n {}".format(MAX_EPOCHS*469),
                           ha="center", va="center", color="white",size=15)

ax.set_title("Grid serch for SGD-HD",size=20)
