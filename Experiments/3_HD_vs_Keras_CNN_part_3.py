...
optm_HD = MyOpts.Adam_HD(alpha_0 = 0.001, beta_1 = 0.9, beta_2=0.999,
                         epsilon = 1e-8, beta= 1e-8)
            

VGG_Net['Adam']['HD']['alpha_epoch'].append(0.001)
VGG_Net['Adam']['HD']['loss'].append(start_loss_train)
VGG_Net['Adam']['HD']['val_loss'].append(start_loss_val)
...
print('\nAdam-HD, alpha_0=0.001, beta= 1e-8 (CIFAR10).')
for epoch in range(EPOCHS):
      t0 = time.time() 
      alphas_means = 0
      for (x_train, y_train) in train_dataset:
        train_step(model, optm_HD, x_train, y_train)
        VGG_Net['Adam']['HD']['alpha_it'].append(optm_HD._get_alpha().numpy())
      ...
      VGG_Net['Adam']['HD']['alpha_epoch'].append(alphas_means/iter_for_epoch)
      VGG_Net['Adam']['HD']['loss'].append(train_loss.result().numpy()*1)
      VGG_Net['Adam']['HD']['val_loss'].append(test_loss.result().numpy()*1)
      VGG_Net['Adam']['HD']['val_acc'].append(test_accuracy.result().numpy()*1)
...
VGG_Net['Adam']['HD']['loss_it'] = VGG_Net['Adam']['HD']['loss_it']  + iteration_loss

save_result(VGG_Net, 'VGG_performances_result_final')
model.save_weights('Adam_HD_final.h5')
