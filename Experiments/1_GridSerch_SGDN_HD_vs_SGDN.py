...
tf.random.set_seed(10001)

print("Versione tensorfolow:", tf.__version__)
print('Grid Serch SGDN')
...
for i in range(6):
  for j in range(7):
    ...
    print_ = True
    if isnan(beta):
        opt = keras.optimizers.SGD(lr=alpha, momentum=0.9, nesterov= True)
    else: 
        opt = MyOpts.SGDN_HD(alpha_0=alpha, mu=0.9, beta=beta)
...
save_result(Grid_np, 'Output_GridSerch_SGDN')
# Grid_np = load_result('Output_GridSerch_SGDN')

fig, ax = plt.subplots(figsize=(18, 8))

xlab = ["$10^{-6}$", "$10^{-5}$", "$10^{-4}$",
        "$10^{-3}$","$10^{-2}$","$10^{-1}$","Ref. SGDN"]
ylab = ["$10^{-1}$", "$10^{-2}$", "$10^{-3}$",
        "$10^{-4}$","$10^{-5}$","$10^{-6}$"]
...
plt.plot([3.5,3.5,4.5,4.5,5.45],[-0.5,1.5,1.5,2.5,2.5], linewidth=2, c='r')
plt.fill([3.5,3.5,4.5,4.5,5.5,5.5],[-0.5,1.5,1.5,2.5,2.5,-0.5],
          c='r', alpha=0.2)
plt.fill([5.5,5.5,6.5,6.5],[3.5,5.5,5.5,3.5],
          c='grey', alpha=1)
text = ax.text(6, 4.5, " More then \n {}".format(MAX_EPOCHS*469),
                            ha="center", va="center", color="white",size=15)

ax.set_title("Grid serch for SGDN-HD",size=20)
