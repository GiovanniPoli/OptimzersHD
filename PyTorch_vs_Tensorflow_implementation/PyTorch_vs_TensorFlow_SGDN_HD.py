...
for i in range(1, n_iterations + 1):
    ...
    print("TensorFlow Loss at iteration {}: {}".format(i, train_loss_tf.result().numpy()*1))
    print("\ngradients (PyTorch):")
    ...
    print("Momentum (PyTorch):")
    print(optimizer_torch.state_dict()['state'][0]['momentum_buffer'])
    print("Momentum (TensorFlow):")
    print([optimizer_tf.get_slot(v, 'vel').numpy().round(4) for v in model_keras.trainable_variables])
    print("\nalpha_t: {}".format(optimizer_torch.state_dict()["param_groups"][0]["lr"].numpy()*1))
