...
optimizer_torch = HdOpts.AdamHD(model_torch.parameters(), lr=learning_rate,
                                hypergrad_lr=beta, betas=(0.9,0.999),
                                eps=1e-8)
...
optimizer_tf = MyOpts.Adam_HD(alpha_0=learning_rate, beta=beta, beta_1=0.9, beta_2=0.999,
                              epsilon=1e-8)
...
# =============================================================================
    print("\n\nPyTorch Loss at iteration {}:    {}".format(i, loss_torch))
    ...
    print("\nFirst moment (PyTorch):")
    print(optimizer_torch.state_dict()['state'][0]['exp_avg'])
    print(optimizer_torch.state_dict()['state'][1]['exp_avg'])
    print("First moment (TensorFlow):")
    print(optimizer_tf.get_slot(model_keras.trainable_variables[0], 'm').numpy().transpose().round(4))
    print(optimizer_tf.get_slot(model_keras.trainable_variables[1], 'm').numpy().transpose().round(4))
    print("\nSecond moment (PyTorch):") 
    print(optimizer_torch.state_dict()['state'][0]['exp_avg_sq'])
    print(optimizer_torch.state_dict()['state'][1]['exp_avg_sq'])
    print("Second moment (TensorFlow):")
    print(optimizer_tf.get_slot(model_keras.trainable_variables[0], 'v').numpy().transpose().round(4))
    print(optimizer_tf.get_slot(model_keras.trainable_variables[1], 'v').numpy().transpose().round(4))
